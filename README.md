# <div align=center> 패스트푸드점 메뉴 추천 시스템 </div>
...
> 데이터베이스와 GUI를 연결한 간소화된 인터페이스이다. tkinter을 사용하여 띄워진 창 속에서 실행되며, 사용자의 입력을 받아 데이터베이스로부터 필요한 데이터를 가져와 출력한다. 자세한 코드는 program.py를 참고하길 바란다.
> #### 서버 인터페이스 Server Interface
> 시스템 관리자가 직접 SQL문을 통해 데이터베이스에서 데이터를 추가 및 삭제하고 입력 정보에 따라 업데이트 되도록 하였다. 자세한 코드는 database.sql을 참고하길 바란다.
